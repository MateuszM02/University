...
(define-type Exp
  ...
  (lamE [xs : (Listof Symbol)] [e : Exp]) ;zmiana
  (appE [e1 : Exp] [e2 : (Listof Exp)])) ;zmiana
...
(define (parse [s : S-Exp]) : Exp
  (cond
    ...
    [(s-exp-match? `{lambda {SYMBOL ...} ANY} s)
      (let ([ss (rest (s-exp->list s))]) ;wyrazenie postaci (list `(x y) `(+ x y))
        (lamE
             (map s-exp->symbol (s-exp->list (first ss))) ;lista nazw zmiennych
             (parse (second ss))))]
    ...
    [(s-exp-match? `{let SYMBOL ANY ANY} s)
     (letE (s-exp->symbol (second (s-exp->list s)))
           (parse (third (s-exp->list s)))
           (parse (fourth (s-exp->list s))))]
    [(s-exp-match? `{SYMBOL ANY ANY} s) ;zmiana
...
(define-type Value
  (numV [n : Number])
  (boolV [b : Boolean])
  (funV [x : (Listof Symbol)] [e : Exp] [env : Env])
...
(define (eval [e : Exp] [env : Env]) : Value
  (type-case Exp e
    ...
    [(appE e1 e2) ;zmiana
     (apply (eval e1 env) (map (lambda (x) (eval x env)) e2))]))

;(extend-env [env : Env] [x : Symbol] [v : Value])
;(funV [x : (Listof Symbol)] [e : Exp] [env : Env])

(define (make-pairs [xs : (Listof Symbol)] [ys : (Listof Value)]) : (Listof (Symbol * Value)) ;NOWE
  (type-case (Listof Symbol) xs
            [empty empty]
            [(cons h t) (cons (pair h (first ys)) (make-pairs t (rest ys)))]))

(define (apply [v1 : Value] [v2 : (Listof Value)]) : Value ;zmiana
  (type-case Value v1
    ;[(funV x b env)
     ;(eval b (extend-env env x v2))]
    [(funV xs b env) ;zmiana ZLE?
      (let ([pair_of_lists (make-pairs xs v2)])
           (eval b
                 (foldl (lambda (x acc) (extend-env acc (fst x) (snd x))) env pair_of_lists)))]
    ;[(primopV f)
     ;(f v2)]
    [(primopV f) ;ZLE
     (map (lambda (x) (f x)) v2)]
