...
#|---------------------------------------------------------------------------------------
funkcja tlumaczaca wyrazenia
---------------------------------------------------------------------------------------|#
;tworzy nazwy nowych zmiennych
(define (name-generator [x : Number]) : String
  (if (<= x 0)       "a"
      (string-append "a" (name-generator (- x 1)))))
;tłumaczy wyrażenia z adresami leksykalnymi na tradycyjną reprezentację z nazwanymi zmiennymi
(define (detranslate [e : ExpA] [n : Number]) : Exp 
  (type-case ExpA e
    [(numA n) (numE n)]
    [(opA o l r)
     (opE o (detranslate l n) (detranslate r n))]
    [(ifA b l r)
     (ifE (detranslate b n)
          (detranslate l n)
          (detranslate r n))]
    [(varA x) ;indeks zmiennej
     (varE (string->symbol (name-generator (- (- x n) 1))))] ;zwraca nowa nazwe zmiennej
    [(letA e1 e2) 
     (letE (string->symbol (name-generator n))
           (detranslate e1 n)
           (detranslate e2 (+ 1 n)))]))
