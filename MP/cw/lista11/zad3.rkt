...
#|-----------------------------------------------------------------------
procedura wypisujaca liste zmiennych wolnych
-----------------------------------------------------------------------|#
...
(define (free-vars [e : Exp]) : (Listof Symbol)
  (type-case Exp e
    ...
    [(letE x e1 e2)
     (append (free-vars e1)
             (free-vars (subst e2 x (numE 0))))]))

;testy na poprawnosci free-vars

(module+ test
  (test (free-vars (parse `2))
        empty)
  (test (free-vars (parse `{let x 3 x}))
        empty)
  (test (free-vars (parse `{let x x 2}))
        (list 'x))
  (test (free-vars (parse `{let x 3 y}))
        (list 'y))
  (test (free-vars (parse `{let x 3 {let y 5 {+ x y}}}))
        empty)
  (test (free-vars (parse `{let x {let y 3 {* y y}} {+ x y}}))
        (list 'y)))
