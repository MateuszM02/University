...
;zad2 - error-ans-monad-macros.rkt
  ...
  (λ (v1 v2)
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
         [(numV n2)
          (valueA (numV (f n1 n2)))] ; zapakowanie value w answer
         [else
  ...
  (λ (v1 v2)
    (type-case Value v1
      [(numV n1)
       (type-case Value v2
         [(numV n2)
          (valueA (boolV (f n1 n2)))] ; zapakowanie value w answer
         [else
          (err 'eval "type error")])] ; zmiana na err
      [else
       (err 'eval "type error")]))) ; zmiana na err
...
(define (eval [e : Exp]) : Answer
  (type-case Exp e
    ...
    [(opE o l r)
     (do ([v-l (eval l)]
          [v-r (eval r)])
       (if (and (equal? (op->proc o) (op-num-num->proc /)) ;test na /0
                (equal? v-r (numV 0)))
           (err 'eval "division by 0")
