  ...
  (type-case Exp e
    ...
    [(setE x e0)
     (begin 
       ...
       (voidV))]
    ;NOWE---------------------------------------------- TO NIE DZIALA JAK TRZEBA
    #|[(whileE war body) ;ZLE, PETLI SIE
     (type-case Value (eval war env)
       ...
       [else
        (error 'eval "type error")])]))|#
    [(whileE cond exp)
     (type-case Value (eval cond env)
         [(boolV b)
          (if b
              (eval exp env) ;ZLE
              (eval exp env))] ;PRAWIE DOBRZE - ZACHOWUJE SIE JAK DO-WHILE, CZYLI COS DROBNEGO DO POPRAWY
         [else (error 'eval "type error - first argument of while must be a boolean value")])]))
;(define (update-env! [env : Env] [x : Symbol] [v : Value]) : Void - LINIA 142/143, MOZE SIE PRZYDAC

;POTENCJALNE TESTY:
;(run `{let x 0 {while {<= x 10} {+ x 1}}}) = 10
;(run `{let x 0 {while {<= x -10} {+ x 1}}}) = 0
    ;--------------------------------------------------
;(trace eval)
